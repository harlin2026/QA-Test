...
import { clickPosNav, clickTextButton, fillByPlaceholder, openPosPage, uniquePhone } from '../helpers/pos';
...
test.describe('E2E POS 會員與存酒', () => {
  ...
  test('2. 新建存酒頁可填寫並取消', async ({ page }) => {
    ...
    await fillByPlaceholder(page, '请输入顾客姓名', 'E2E巡览');
    await fillByPlaceholder(page, '请输入手机号', uniquePhone());
    await expect(page.getByText(/确认存酒登记|酒品名称/).first()).toBeVisible();
